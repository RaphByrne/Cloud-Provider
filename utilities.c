...
//used to log into bank/provider
//Sends a login message and waits for the server to respond with verification
...
//Compares non null-terminated character arrays
//returns 1 if equal up to min(l1,l2) 
//0 otherwise
...
//testing function for printing hashes
...
//Send a null-terminated string over bio
...
//complete working. Seems to have problem with XDRed things
...
//Does a stat to get the size of the file in bytes
...
//File encryption/decryption function
//Doesn't full work on lab machines
//do_crypt controls mode: 1 for enc, 0 for dec
...
//Experiment encryption function similar to that for files but for character buffers instead.
//Was to be used as an alternative to signing the token verification with the public key
//but also couldn't get it working. Not used in the end
...
//Sends the local file 'filename' over the bio
//Checks with the server first to see if they can receive a file with that name
//an effort is maintained to preserve filenames between systems
...
//gets a null-terminated string from the bio
...
//get some characters from the bio
...
//send from character to the bio
...
//read a message_client into m from the bio
